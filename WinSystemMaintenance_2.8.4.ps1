```powershell
# WinSystemMaintenance_2.8.4.ps1
...
# Optimization Section - Optimizes drives based on media type (TRIM for SSDs, defrag for HDDs) using specific drive letters.
if (Confirm-Action -Message "Proceed with Drive Optimization?" -Default "Y") {
    Write-Log "Starting Drive Optimization..."
    # Check for SSD drives and optimize only selected SSDs with TRIM.
    if ($ssdDrives) {
        Write-Log "SSD drives detected: $($ssdDrives -join ', ')"
        Write-Host "`nSSD drives detected: $($ssdDrives -join ', ')" -ForegroundColor Yellow
        if (Confirm-Action -Message "Proceed with TRIM optimization for SSD drives ($($ssdDrives -join ', '))?" -Default "Y") {
            foreach ($drive in $ssdDrives) {
                $defragCmd = "defrag $drive`: /O /V"
                Write-Log "Starting: SSD Optimization for $drive`: ($defragCmd)"
                Write-Host "`nRunning: SSD Optimization for $drive`: ($defragCmd)" -ForegroundColor Cyan
                Write-Log "Command: $defragCmd"
                $defragOutput = Invoke-Expression "$defragCmd 2>&1"
                ...
                if ($defragOutput -match "optimized|trimmed") {
                    Write-Log "Result: TRIM completed for $drive`:."
                    Write-Host "`nResult: TRIM completed for $drive`:" -ForegroundColor Green
                    $defragOptimizations++
                } elseif ($defragOutput -match "error|failed") {
                    Write-Log "Result: Errors encountered for $drive`:."
                    Write-Host "`nResult: Errors encountered for $drive`:" -ForegroundColor Yellow
                    $defragErrors++
                }
            }
        } else {
            Write-Log "Result: Skipped SSD optimization."
            Write-Host "`nResult: Skipped SSD optimization" -ForegroundColor Green
        }
    }
    # Check for non-SSD drives and optimize only selected HDDs with defrag.
    if ($nonSsdDrives) {
        ...
        if (Confirm-Action -Message "Proceed with defrag for non-SSD drives ($($nonSsdDrives -join ', '))? (may be time-consuming)" -Default "N") {
            foreach ($drive in $nonSsdDrives) {
                $defragCmd = "defrag $drive`: /O /V"
                Write-Log "Starting: Non-SSD Optimization for $drive`: ($defragCmd)"
                Write-Host "`nRunning: Non-SSD Optimization for $drive`: ($defragCmd)" -ForegroundColor Cyan
                Write-Log "Command: $defragCmd"
                $defragOutput = Invoke-Expression "$defragCmd 2>&1"
                $defragOutput | Out-File -FilePath $logFile -Append -Encoding UTF8
                Write-Host $defragOutput
                if ($defragOutput -match "defragmented") {
                    Write-Log "Result: Defragmentation completed for $drive`:."
                    Write-Host "`nResult: Defragmentation completed for $drive`:" -ForegroundColor Green
                    $defragOptimizations++
                } elseif ($defragOutput -match "error|failed") {
                    Write-Log "Result: Errors encountered for $drive`:."
                    Write-Host "`nResult: Errors encountered for $drive`:" -ForegroundColor Yellow
            ...
            Write-Host "`nResult: Skipped non-SSD defragmentation" -ForegroundColor Green
        }
    }
    # Handle case where no drives are eligible for optimization.
...
if (Confirm-Action -Message "Proceed with Temp File Cleanup?" -Default "Y") {
    ...
    # Function to calculate folder size - Measures size of directories for cleanup reporting.
...
if (Confirm-Action -Message "Proceed with Hardware and Driver Checks?" -Default "Y") {
    ...
    # Check disk health using Get-PhysicalDisk.
    ...
    # Check SMART attributes for read/write errors and wear.
    ...
    # Check for problematic drivers using Win32_PnPSignedDriver.
...
if (Confirm-Action -Message "Proceed with Memory and Update Checks?" -Default "Y") {
    ...
    # Check for failed Windows Updates using Get-HotFix.
    ...
    # Schedule memory diagnostic for non-server systems if user agrees.
