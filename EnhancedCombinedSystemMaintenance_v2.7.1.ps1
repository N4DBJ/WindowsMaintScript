...
if (Confirm-Action -Message "Proceed with Log Analysis?" -Default "Y") {
    ...
    Write-Log "Script Version: $scriptVersion"
    Write-Log "OS Maintenance: Errors=$osErrors, Repairs=$osRepairs"
    Write-Log "Disk Maintenance: Errors=$diskErrors, Repairs=$diskRepairs"
    Write-Log "Shadow Copy Maintenance: Corrupted=$corruptedShadows, Deleted=$shadowsDeleted"
    Write-Log "Drive Optimization: Errors=$defragErrors, Optimizations=$defragOptimizations"
    Write-Log "Temp File Cleanup: Freed=$totalBytesFreed MB, Errors=$cleanupErrors"