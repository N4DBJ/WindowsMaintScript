...
Write-Host "Script Version: 2.5.9" -ForegroundColor Cyan
...
"Script Version: 2.5.9" | Out-File -FilePath $logFile -Encoding UTF8
Write-Log "Script Version: 2.5.9"
...
if (Confirm-Action -Message "Proceed with Disk Maintenance (CHKDSK, fsutil, Contig)?" -Default "Y") {
    ...
    foreach ($drive in $ntfsDrives.DriveLetter) {
        ...
        if (Confirm-Action -Message "Proceed with CHKDSK Scan on $drivePath ($chkdscScanCmd)?" -Default "Y") {
...
if (Confirm-Action -Message "Proceed with Hardware and Driver Checks?" -Default "Y") {
    ...
    Write-Log "Command: Get-WmiObject Win32_PnPSignedDriver"
...
if (Confirm-Action -Message "Proceed with Log Analysis?" -Default "Y") {
    ...
    Write-Log "Log Analysis Summary:"
    Write-Log "Pending Updates: $pendingUpdates"
    Write-Log "Pending Restarts: $($pendingRestartSources -join ', ')"
    Write-Log "DISM.log Errors/Failures: $dismErrors"
    Write-Log "DISM.log Repairs: $dismRepairs"
    Write-Log "CBS.log Errors/Failures: $cbsErrors"
    Write-Log "CBS.log Repairs: $cbsRepairs"
    foreach ($drivePath in $contigErrorsPerDrive.Keys) {
        $driveType = ($ntfsDrives | Where-Object { $_.DriveLetter -eq $drivePath[0] }).MediaType
        Write-Log "Contig Errors on $drivePath ($driveType): $($contigErrorsPerDrive[$drivePath])"
        Write-Log "Contig MFT Fragments on $drivePath ($driveType): $($contigFragmentsPerDrive[$drivePath])"
    }
    Write-Log "fsutil Corrupted Drives: $fsutilCorruptedDrives"
    Write-Log "Corrupted Shadow Copies: $shadowCorrupted"
    Write-Log "Defrag Errors: $defragErrors"
    Write-Log "Defrag Optimizations: $defragOptimizations"
    Write-Log "Disk Errors (Events): $diskErrors"
    Write-Log "Driver Errors (Events): $driverErrors"
    Write-Log "Hardware Errors (Events): $hardwareErrors"
    Write-Log "VSS Errors (Events): $vssErrors"
    Write-Log "Temp Cleanup Errors: $cleanupErrors"
    Write-Log "Temp Cleanup Freed: $totalBytesFreed MB"
    Write-Log "Defender Errors: $defenderErrors"
    ...
    Write-Host "Corrupted Shadow Copies: $shadowCorrupted"
    Write-Host "Defrag Errors: $defragErrors"
    Write-Host "Defrag Optimizations: $defragOptimizations"
    Write-Host "Disk Errors (Events): $diskErrors"
    Write-Host "Driver Errors (Events): $driverErrors"
    Write-Host "Hardware Errors (Events): $hardwareErrors"
    Write-Host "VSS Errors (Events): $vssErrors"
    Write-Host "Temp Cleanup Errors: $cleanupErrors"
    Write-Host "Temp Cleanup Freed: $totalBytesFreed MB"
    Write-Host "Defender Errors: $defenderErrors"

    # Recommendations based on analysis
    if ($dismErrors -gt 0 -or $cbsErrors -gt 0) {
        Write-Log "Recommendation: Review DISM.log and CBS.log for details."
        Write-Host "Recommendation: Review DISM.log and CBS.log for details" -ForegroundColor Yellow
    }
    if ($fsutilCorruptedDrives -gt 0) {
        Write-Log "Recommendation: Run full CHKDSK /f /r on affected drives."
        Write-Host "Recommendation: Run full CHKDSK /f /r on affected drives" -ForegroundColor Yellow
    }
    if ($shadowCorrupted -gt 0) {
        Write-Log "Recommendation: Verify VSS configuration and consider recreating shadow copies."
        Write-Host "Recommendation: Verify VSS configuration and consider recreating shadow copies" -ForegroundColor Yellow
    }
    if ($driverErrors -gt 0) {
        Write-Log "Recommendation: Update or rollback drivers via Device Manager."
        Write-Host "Recommendation: Update or rollback drivers via Device Manager" -ForegroundColor Yellow
    }
    if ($hardwareErrors -gt 0) {
        Write-Log "Recommendation: Run full hardware diagnostics (e.g., CrystalDiskInfo)."
        Write-Host "Recommendation: Run full hardware diagnostics (e.g., CrystalDiskInfo)" -ForegroundColor Yellow
    }
} else {
