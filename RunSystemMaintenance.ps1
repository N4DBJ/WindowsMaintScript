...
#              repair source for DISM /restorehealth, and MFT defragmentation with Contig on a user-specified drive.
...
$commands = @(
    ...
    @{Command = "chkdsk /scan c:"; Description = "CheckDisk - Scan C: drive for errors"}
)
...
foreach ($cmd in $commands) {
    ...
    if (Confirm-Action -Message "Proceed with $($cmd.Description)?" -Default "Y") {
        ...
        try {
            # Execute command, capture stdout/stderr, display to console, and log to file
...
if (Confirm-Action -Message "Proceed with analyzing DISM.log, CBS.log, Event Log, and Contig output for errors and repairs?" -Default "Y") {
    ...
    $chkdskRepairPattern = "repaired|fixed|corrected"
