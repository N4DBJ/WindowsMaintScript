...
#              real-time progress output for sfc /scannow, additional CHKDSK /f /sdcleanup command, and optional
#              download of Sysinternals Suite if not found at C:\SysinternalsSuite.
...
# Check for Sysinternals Suite and offer to download if not found
...
if (-not (Test-Path $sysinternalsPath)) {
    ...
    if (Confirm-Action -Message "Download and extract Sysinternals Suite to $sysinternalsPath and add to PATH?" -Default "N") {
        Write-Host "Downloading Sysinternals Suite..." -ForegroundColor Green
        "----- Downloading Sysinternals Suite at $(Get-Date) -----" | Out-File -FilePath $logFile -Append
        try {
            $zipUrl = "https://download.sysinternals.com/files/SysinternalsSuite.zip"
            $tempZip = [System.IO.Path]::GetTempFileName() + ".zip"
            Invoke-WebRequest -Uri $zipUrl -OutFile $tempZip
            Write-Host "Extracting to $sysinternalsPath..." -ForegroundColor Green
            ...
            Write-Host "Adding $sysinternalsPath to system PATH..." -ForegroundColor Green
            ...
            "Successfully downloaded and extracted Sysinternals Suite to $sysinternalsPath and added to PATH" | Out-File -FilePath $logFile -Append
        } catch {
            Write-Host "Error downloading or extracting Sysinternals Suite: $_" -ForegroundColor Red
            "Error downloading or extracting Sysinternals Suite: $_" | Out-File -FilePath $logFile -Append
        }
        "----- End of Sysinternals Suite Download -----" | Out-File -FilePath $logFile -Append
    } else {
        Write-Host "Skipped Sysinternals Suite download. Contig may not be available." -ForegroundColor Yellow
        "Skipped Sysinternals Suite download" | Out-File -FilePath $logFile -Append
    }
}
...
if (-not (Get-Command "contig" -ErrorAction SilentlyContinue)) {
    Write-Host "Contig not found in PATH. Please download from Sysinternals (https://learn.microsoft.com/sysinternals/downloads/contig) or ensure $sysinternalsPath is in PATH." -ForegroundColor Red
    "Contig not found. Download from https://learn.microsoft.com/sysinternals/downloads/contig or ensure $sysinternalsPath is in PATH." | Out-File -FilePath $logFile -Append
} else {
