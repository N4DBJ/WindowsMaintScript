...
# Description: Fixed PowerShell parsing errors in Temp File Cleanup section by using ${} for variables with colons.
...
if (Confirm-Action -Message "Proceed with expanded log analysis?" -Default "Y") {
    ...
    $totalErrors = $dismErrors + $cbsErrors + $chkdskErrors + ($contigErrorsPerDrive.Values | Measure-Object -Sum).Sum + $fsutilCorruptedDrives + $shadowCorrupted + $defragErrors + $diskErrors + $driverErrors + $hardwareErrors + $vssErrors + $cleanupErrors
