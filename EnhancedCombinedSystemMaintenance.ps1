...
# Description: Fixed PowerShell parsing errors in Shadow Copy Maintenance section by using ${shadowVolume} in Write-Host statements to handle colons in shadow volume names (e.g., \Device\HarddiskVolumeShadowCopy3).
#              Changed Contig command from 'contig -v -s <drive>:\$MFT' to 'contig -v -accepteula <drive>:\$MFT' to remove recursive flag (-s) causing issues and add -accepteula for first-run license acceptance.
