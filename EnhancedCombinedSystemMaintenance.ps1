...
# Description: Added Initial System State Check section to detect pending Windows Updates (via PSWindowsUpdate or Get-HotFix) and pending restarts (Windows Update, Component-Based Servicing, Pending File Rename Operations, SCCM).
#              Prompts user to restart if pending updates or restarts are detected (default "Y"). If declined, proceeds with warning.
#              Moved pending reboot check from Memory and Update Checks to Initial System State Check.
#              Updated Log Analysis to include pending updates and restarts in summary.
#              Previous changes from 2.5.4:
#              - Added real-time DISM progress display on console using Write-Progress for percentage updates (e.g., [========== 18.1% ]).
#              - Cleaned up DISM log output to exclude repetitive progress lines, logging only the final progress percentage and significant messages.
#              - Added checks for Windows Defender service (WinDefend) status and third-party antivirus software before running Start-MpScan.
#              - Fixed PowerShell parsing errors in Shadow Copy Maintenance section by using ${shadowVolume}.
#              - Changed Contig command to remove recursive flag and add -accepteula.
#              - Added result-driven prompts for DISM commands, on-screen section summaries, temp file cleanup, and fixed SSD vs. HDD detection.
...
try {
    # Attempt to install PSWindowsUpdate module if not present
    ...
    # Fallback to Get-HotFix and registry check
        ...
        if (Confirm-Action -Message "Proceed with $($cmd.Description)?" -Default "Y") {
            ...
            try {
                if ($cmd.Command -eq "sfc /scannow") {
                    ...
                    $tempFile = [System.IO.Path]::GetTempFileName()
                    $process = Start-Process -FilePath "dism" -ArgumentList ($cmd.Command -split " ") -NoNewWindow -RedirectStandardOutput $tempFile -RedirectStandardError $tempFile -PassThru
                    $lastPercentage = 0
                    ...
                    # Filter output for logging: exclude repetitive progress lines, keep final percentage and other messages
                    $progressPattern = "\[\=+\s*\d+\.\d+%\s*\]"
                    $finalPercentage = $null
                    $filteredOutput = @()
                    foreach ($line in ($output -split "`n")) {
                        if ($line -match $progressPattern) {
                            $finalPercentage = $line  # Keep the last progress line
                        } elseif ($line -and $line -notmatch $progressPattern) {
                            $filteredOutput += $line  # Keep non-progress lines
                        }
                    }
                    if ($finalPercentage) {
                        $filteredOutput += $finalPercentage  # Append the final progress percentage
                    }
