...
# Description: Fixed DISM command execution error in OS Maintenance Section where Start-Process passed the entire command string
#              including "dism" as an argument, causing "Error: 87". Updated $osCommands to store only arguments for DISM,
#              excluding "dism" prefix, and adjusted Start-Process to use correct argument list.
#              Previous changes from 2.5.6:
#              - Fixed error in OS Maintenance Section where Start-Process failed due to RedirectStandardOutput and RedirectStandardError
...
if (Confirm-Action -Message "Proceed with OS Maintenance (DISM and SFC)?" -Default "Y") {
    $osCommands = @(
        @{Command = "/online /cleanup-image /checkhealth"; Description = "CheckHealth - Check for component store corruption"; Executable = "dism"},
        @{Command = "/online /cleanup-image /scanhealth"; Description = "ScanHealth - Perform a deeper scan for corruption"; Executable = "dism"},
        @{Command = if ($repairSource) { "/online /cleanup-image /restorehealth /source:$repairSource" } else { "/online /cleanup-image /restorehealth" }; Description = "RestoreHealth - Repair the component store"; Executable = "dism"},
        @{Command = "/online /cleanup-image /AnalyzeComponentStore"; Description = "AnalyzeComponentStore - Analyze component store size"; Executable = "dism"},
        @{Command = "/online /cleanup-image /StartComponentCleanup"; Description = "StartComponentCleanup - Clean up superseded components"; Executable = "dism"},
        @{Command = "/scannow"; Description = "SFC Scan - Scan and repair system files"; Executable = "sfc"}
    )
    ...
    foreach ($cmd in $osCommands) {
        ...
        if (Confirm-Action -Message "Proceed with $($cmd.Description)?" -Default "Y") {
            Write-Log "Executing: $($cmd.Executable) $($cmd.Command)"
            try {
                ...
                } elseif ($cmd.Description -eq "SFC Scan - Scan and repair system files") {
                    if ($combinedOutput -match "found.*and repaired") {
                        Write-Host "`nSFC Scan Result: Repairs completed" -ForegroundColor Green
                        Write-Log "SFC Scan: System file repairs completed"
                        $osRepairs++
                    } elseif ($combinedOutput -match "corrupt|cannot repair" -or $errorOutput) {
                        Write-Host "`nSFC Scan Result: Errors encountered" -ForegroundColor Yellow
                        Write-Log "SFC Scan: Errors during scan"
                        $osErrors++
                    }
                }
